...
export function ServicesSection() {
  return (
    <section className="py-24 sm:py-32">
      <SectionHeading
        eyebrow="Xidmətlər"
        title="Hazır template hissi verməyən, biznes nəticəsinə işləyən premium sayt foundation-ları qururam."
        description="Xidmət adı ilə kifayətlənmirik. Hər sayt formatı konkret olaraq nəyi yaxşılaşdıracağını əvvəlcədən göstərməlidir: görünüşü, etibarı, sürəti və sorğu axınını."
        action={
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-12">
          {studioServices.map((service, index) => (
            <Reveal key={service.title} delay={index * 0.03} className={cn(getCardSpan(index))}>
              <div className="group h-full rounded-[30px] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.045),rgba(255,255,255,0.02))] p-6 transition-all duration-500 hover:-translate-y-1 hover:border-[var(--color-accent)]/35 hover:shadow-[0_22px_70px_rgba(0,0,0,0.22)] sm:p-7">
                ...
                <h3 className="mt-8 max-w-xl font-display text-3xl tracking-[-0.05em] text-[var(--color-text)]">
                  {service.title}
                </h3>
