    ...
    <section className="py-24 sm:py-32">
      ...
      <Container>
        <div className="grid gap-6 xl:grid-cols-[1.04fr_0.96fr]">
          <Reveal>
            <div className="rounded-[36px] border border-[var(--color-accent-secondary)]/30 bg-[linear-gradient(180deg,rgba(0,230,118,0.08),rgba(20,184,166,0.1),rgba(11,31,24,0.92))] p-7 sm:p-9">
              <div className="flex flex-wrap items-center gap-3">
                ...
                {socialProofPoints.map((item) => (
                  <div
                    key={item}
                    className="rounded-full border border-[color:var(--color-border)] bg-[rgba(167,243,208,0.04)] px-3 py-1 text-[10px] uppercase tracking-[0.24em] text-[var(--color-muted)]"
                  >
                    {item}
                  </div>
                ))}
              ...
              <div className="mt-10 border-t border-[color:rgba(167,243,208,0.08)] pt-6">
          ...
          <div className="grid gap-6">
            {supportingTestimonials.map((item, index) => (
              <Reveal key={item.name + item.company} delay={index * 0.05}>
                <div className="rounded-[30px] border border-[color:var(--color-border)] bg-[rgba(11,31,24,0.86)] p-6 sm:p-7 transition-colors duration-300 hover:border-[var(--color-accent-secondary)]/30">
                  <p className="text-lg leading-8 text-[var(--color-text)]">“{item.quote}”</p>
                  <div className="mt-7 border-t border-[color:rgba(167,243,208,0.08)] pt-5">
