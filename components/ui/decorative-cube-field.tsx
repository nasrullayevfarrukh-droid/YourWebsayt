...
function ParticleSwarm({ className, dense = false }: { className?: string; dense?: boolean }) {
  const positions = dense
    ? ["left-0 top-4", "left-6 top-0", "left-16 top-5", "right-8 top-2", "right-2 top-10", "left-10 bottom-1", "right-12 bottom-0", "right-4 bottom-8"]
    : ["left-0 top-5", "left-7 top-0", "right-8 top-4", "right-1 top-11", "left-12 bottom-3", "right-10 bottom-0"];

  return (
    <div className={cn("absolute h-24 w-28", className)}>
      {positions.map((position, index) => (
        <span
          key={position}
          className={cn(
            "cube-particle absolute rounded-full bg-[rgba(220,255,246,0.82)] shadow-[0_0_18px_rgba(106,255,214,0.34)]",
            index % 4 === 0 ? "h-2 w-2" : index % 2 === 0 ? "h-1.5 w-1.5" : "h-1 w-1",
            position
          )}
    ...
    >
      {showLeft ? (
        <div className="absolute -left-4 top-10">
          ...
          <ParticleSwarm className="left-10 top-10" dense />
        </div>
      ) : null}
      ...
      {showRight ? (
        <div className={cn("absolute right-0", variant === "showcase" ? "top-24" : "bottom-8")}>
          ...
          <ParticleSwarm className="right-10 top-8" dense />
        </div>
      ) : null}
      ...
      {variant === "showcase" ? (
        <div className="absolute left-8 bottom-8">
          ...
          <ParticleSwarm className="left-12 top-6" />
        </div>
      ) : null}
    </div>
  );
